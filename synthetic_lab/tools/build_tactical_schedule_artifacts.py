...
from transcendence_lab.tactical_schedule import build_trace_tactical_temporal_schedule
...
def build(output_directory: Path) -> list[Path]:
    ...
    schedule_suite = read_json(
        LAB_ROOT / "scenarios" / "tactical_schedule_matrix_v0.1Q.json"
    )
    battle4 = build_trace_tactical_temporal_schedule(trace)
    matrix = run_tactical_schedule_matrix(trace, baseline_suite, schedule_suite)
    outputs = [
        output_directory / "BATTLE4_EILHART_TACTICAL_TEMPORAL_SCHEDULE_v0.1Q.json",
        output_directory / "TACTICAL_TEMPORAL_SCHEDULE_MATRIX_REPORT_v0.1Q.json",
    ]
